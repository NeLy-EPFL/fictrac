...
Building a map of the surface and matching each observed frame to it can ensure long-term consistency (that is, 1 full rotation of the ball being counted as exactly 360 degrees), but can cause poor performance when a perfect surface map is not known *a priori*. In instances where a perfect surface map is not available and you don't mind if a full rotation might be counted as slightly more or slightly less than 360 degrees, `accumulate_map` can be set to `n` to disable long-term surface mapping and instead have FicTrac focus on accurately measuring local (frame-to-frame) rotations of the ball.
